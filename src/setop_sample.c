...
main(int argc, char **argv)
{
  grn_ctx ctx;
  grn_obj *db;
  grn_obj *table_1, *column_1;
  grn_obj *table_2, *column_2;
  grn_id id;
  grn_obj *result_1;
  grn_obj *result_2;
  ...
  table_1 = table_create(&ctx, "data_1");
  column_1 = column_create(&ctx, table_1, "column");
  id = record_insert(&ctx, table_1, column_1, "groonga", "groonga world");
  printf("add record:");
  record_print(&ctx, column_1, id);
  lexicon_create(&ctx, table_1, column_1, "lexicon_1");

  result_1 = table_select_by_filter(&ctx, table_1, "column @ \"groonga\"");
  printf("hit records:\n");
  column_print(&ctx, result_1, "column");

  table_2 = table_create(&ctx, "data_2");
  column_2 = column_create(&ctx, table_2, "column");
  id = record_insert(&ctx, table_2, column_2, "mroonga", "mroonga world");
  printf("add record:");
  record_print(&ctx, column_2, id);
  lexicon_create(&ctx, table_2, column_2, "lexicon_2");

  result_2 = table_select_by_filter(&ctx, table_2, "column @ \"mroonga\"");
  printf("hit records:\n");
  column_print(&ctx, result_2, "column");

  //テーブル1←テーブル2の破壊的操作しか実装されていない
  //永続テーブルなら集合演算できたけど、検索結果のハッシュテーブルだとうまく集合演算できない
  //やり方が間違っているのか、対応していないのかはわからない
  grn_table_setoperation(&ctx, table_1, table_2, table_1, GRN_OP_OR);
  printf("merge records:\n");
  //キー以外のカラムは集合演算されない
  column_print(&ctx, table_1, "_key");
